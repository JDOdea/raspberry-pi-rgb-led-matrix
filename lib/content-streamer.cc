...
// Pro-c++11 helper
#define STATIC_ASSET(msg, c) typedef int static_asset_##msg[(c) ? 1 : -1]
    ...
    namespace
    {
        ...
        // the Respberry Pi, but also x86) so it is possible to create streams easily
        ...
        STATIC_ASSET(file_header_size_changed, sizeof(FrameHeader) == 32);
    }
    ...
    {
        if (state_ == STREAM_AT_BEGIN && !ReadFileHeader(*frame))
            return false;
        if (state_ != STREAM_READING)
            return false;
        ...
        // ahead past this header (both headers are designed to be the same size).
    ...
    bool StreamReader::ReadFileHeader(const FrameCanvas &frame)
    {
        ...
        if ((int)header.width != frame.width() || (int)header.height != frame.height())
        {
