...
// between that and the Canvas. This is a quick set of things to get started
// (and nicely self-contained).
...
namespace rgb_matrix
{
    ...
    public:
        ...
        // does not exist.
        int CharactersWidth(uint32_t unicode_codepoint) const;
        ...
        int DrawGlyph(Canvas *c, int x, int y, const Color &color, uint32_t unicode_codepoint) const;

        // Create a new font derived from this font, which represents an outline
        // of the original font, essentially pixels tracing around the original letter.
    ...
    // organized as rgb or bgr. This the size of the buffer needs to be exactly
    // (3 * image_width * image_height) bytes.
    //
    // The "image_buffer" parameters contain the data, "buffer_size_butes" is the
    // size in butes.
    //
    // If "is_bgr" is true, the buffer is treated as BGR pixel arrangement instead
    // of RGB.
    // Returns 'true' if image was shown within canvas.
    bool SetImage(Canvas *c, int canvas_offset_x, int canvas_offset_y, const uint8_t *image_buffer, size_t buffer_size_bytes, int image_width, int image_height, bool is_bgr);
    ...
    // The text is a standard NUL terminated C-string encoded in UTF-*.
    // "font", "x", "y", "color", and "background_color" are same as DrawText().
    // "kerning_offset" allows for additional spacing between characters (can be negative).
