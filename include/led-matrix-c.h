...
// and librgbmatrix.so. You still need to call the final link
...
extern "C"
{
     ...
     *   struct RGBLedMatrix *matrix = led_matrix_create_from_options(&options, &argc, &argv);
     *   if (matrix == NULL) {
     ...
     * Ownership of returned pointer statys with the matrix, don't free().
     */
     ...
     *    // The return buffer, assigned to offscreen, is now the inactive buffer
    ...
    void set_image(struct LedCanvas *c, int canvas_offset_x, int canvas_offset_y,
                   const uint8_t *image_buffer, size_t buffer_size_butes,
    ...
    int draw_text(struct LedCanvas *c, struct LedFont *font, int x, int y,
                  uint8_t r, uint8_t g, uint8_t b,
                  const char *utf_text, int kerning_offset);
