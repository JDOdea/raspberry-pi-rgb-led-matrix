                              ...
                              std::string *err_msg)
{
    ...
    if (fill_width && fill_height)
    {
        ...
        const float larget_fraction = (width_fraction > height_fraction)
                                          ? width_fraction
                                          : height_fraction;
        target_width = (int)roundf(larget_fraction * img_width);
        target_height = (int)roundf(larget_fraction * img_height);
    }
    else if (fill_height)
    {
    ...
    {
        // Ditto vertical. Make things fit in horizontal space.
...
static int usage(const char *progname)
{
    ...
    fprintf(stderr,
            ...
            "-w for static images; -t/-1 for animations.\n"
            "Animated gifs: If both -1 and -1 are given, "
...
int main(int argc, char *argv[])
{
    ...
    // If started with 'sudo': make sure to drop privileges to the same user
    ...
    while ((opt = getopt(argc, argv, "w:t:l:fr:c:P:LhCR:sO:V:D:m")) != -1)
    {
        ...
        case 's':
            ...
            break;
        case 'r':
            fprintf(stderr, "Instead of deprecated -r, use --led-rows=%s.\n",
                    optarg);
            matrix_options.rows = atoi(optarg);
            break;
        case 'c':
            fprintf(stderr, "Instead of deprecated -c, use --led-chain=%s.\n",
                    optarg);
    ...
    for (int imgarg = optind; imgarg < argc; ++imgarg)
    {
        ...
        {
            // If not an image, see if it is one of our streams.
